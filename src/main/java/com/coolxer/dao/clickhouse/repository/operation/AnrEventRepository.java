...
public interface AnrEventRepository extends JpaRepository<AnrEvent, String> {
    @Query(nativeQuery = true,
            value = "SELECT * FROM operation_anr_event ORDER BY update_time ASC",
            countQuery = "SELECT count(*) FROM operation_anr_event")
    Page<ApiCallEvent> findByPage(Pageable pageable);

    @Query(nativeQuery = true,
            value = "SELECT * FROM operation_anr_event WHERE " +
            ...
            countQuery = "SELECT count(*) FROM operation_anr_event WHERE " +
                    "(:id IS NULL OR :id = '' OR id = :id) AND " +
                    "(:userId IS NULL OR :userId = '' OR user_id = :userId) AND " +
                    "(:startId IS NULL OR :startId = '' OR start_id = :startId) AND " +
                    "(:pagePath IS NULL OR :pagePath = '' OR page_path = :pagePath) AND " +
                    ...
                    "(:updateTimeStart IS NULL OR :updateTimeEnd IS NULL OR (update_time >= :updateTimeStart AND update_time <= :updateTimeEnd)) AND " +
                    "(:insertTimeStart IS NULL OR :insertTimeEnd IS NULL OR (insert_time >= :insertTimeStart AND insert_time <= :insertTimeEnd))")
    Page<AnrEvent> findByPageAsc(Pageable pageable,
                                 @Param("id") String id,
    ...
    @Query(nativeQuery = true,
            ...
            countQuery = "SELECT count(*) FROM operation_anr_event WHERE " +
                    ...
                    "(:updateTimeStart IS NULL OR :updateTimeEnd IS NULL OR (update_time >= :updateTimeStart AND update_time <= :updateTimeEnd)) AND " +
                    "(:insertTimeStart IS NULL OR :insertTimeEnd IS NULL OR (insert_time >= :insertTimeStart AND insert_time <= :insertTimeEnd))")
    Page<AnrEvent> findByPageDesc(Pageable pageable,
                                  @Param("id") String id,
                                  @Param("userId") String userId,
                                  @Param("startId") String startId,
                                  @Param("pagePath") String pagePath,
    ...
    @Query(value = "SELECT count(*) FROM operation_anr_event WHERE toDate(insert_time) = toDate(now())", nativeQuery = true)
    long countTodayIncrease();

    @Query(nativeQuery = true, value = "SELECT toStartOfDay(insert_time) AS group_key, COUNT(*) AS count FROM operation_anr_event WHERE insert_time >= today() - INTERVAL 1 WEEK GROUP BY toStartOfDay(insert_time) ORDER BY group_key")
    List<Map<String, Object>> countByDateOfWeek();

    @Query(nativeQuery = true,
            value = "SELECT DISTINCT id FROM operation_anr_event WHERE id IS NOT NULL AND (:searchTerm IS NULL OR :searchTerm = '' OR id LIKE concat('%', :searchTerm, '%'))")
    List<String> findLikeIds(@Param("searchTerm") String searchTerm);

    @Query(nativeQuery = true,
            value = "SELECT DISTINCT page_name FROM operation_anr_event WHERE page_name IS NOT NULL AND (:searchTerm IS NULL OR :searchTerm = '' OR page_name LIKE concat('%', :searchTerm, '%'))")
    List<String> findLikePageNames(@Param("searchTerm") String searchTerm);

    @Query(nativeQuery = true,
            value = "SELECT DISTINCT page_path FROM operation_anr_event WHERE page_path IS NOT NULL AND (:searchTerm IS NULL OR :searchTerm = '' OR page_path LIKE concat('%', :searchTerm, '%'))")
