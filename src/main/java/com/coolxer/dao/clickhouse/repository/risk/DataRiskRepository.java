 ...
 * 数据安全风险数据访问接口
 */
public interface DataRiskRepository extends JpaRepository<DataRisk, String> {

    /**
     * 分页查询数据安全风险（按更新时间升序）
     */
    @Query(nativeQuery = true,
            value = "SELECT * FROM risk_data ORDER BY update_time ASC",
            countQuery = "SELECT count(*) FROM risk_data")
    Page<DataRisk> findByPage(Pageable pageable);

    /**
     * 多条件分页查询（升序）
     */
    @Query(nativeQuery = true,
            value = "SELECT * FROM risk_data WHERE " +
            ...
            countQuery = "SELECT count(*) FROM risk_data WHERE " +
                    "(:id IS NULL OR :id = '' OR id = :id) AND " +
                    "(:userId IS NULL OR :userId = '' OR user_id = :userId) AND " +
                    "(:startId IS NULL OR :startId = '' OR start_id = :startId) AND " +
                    "(:assetId IS NULL OR :assetId = '' OR asset_id = :assetId) AND " +
                    ...
                    "(:updateTimeStart IS NULL OR :updateTimeEnd IS NULL OR (update_time >= :updateTimeStart AND update_time <= :updateTimeEnd)) AND " +
                    "(:insertTimeStart IS NULL OR :insertTimeEnd IS NULL OR (insert_time >= :insertTimeStart AND insert_time <= :insertTimeEnd))")
    Page<DataRisk> findByPageAsc(Pageable pageable,
                                 @Param("id") String id,
    ...
    @Query(nativeQuery = true,
            ...
            countQuery = "SELECT count(*) FROM risk_data WHERE " +
                    ...
                    "(:updateTimeStart IS NULL OR :updateTimeEnd IS NULL OR (update_time >= :updateTimeStart AND update_time <= :updateTimeEnd)) AND " +
                    "(:insertTimeStart IS NULL OR :insertTimeEnd IS NULL OR (insert_time >= :insertTimeStart AND insert_time <= :insertTimeEnd))")
    Page<DataRisk> findByPageDesc(Pageable pageable,
                                  @Param("id") String id,
                                  @Param("userId") String userId,
                                  @Param("startId") String startId,
                                  @Param("assetId") String assetId,
     ...
     */
    @Query(nativeQuery = true,
            value = "SELECT DISTINCT arrayJoin(label) as distinct_label FROM risk_data WHERE label IS NOT NULL AND label != [] order by distinct_label")
     ...
     */
    @Query(nativeQuery = true,
            value = "SELECT id FROM risk_data WHERE id IS NOT NULL AND (:searchTerm IS NULL OR :searchTerm = '' OR id LIKE concat('%', :searchTerm, '%'))")
    List<String> findLikeIds(@Param("searchTerm") String searchTerm);

    @Query(nativeQuery = true,
            value = "SELECT DISTINCT type FROM risk_data WHERE type IS NOT NULL AND (:searchTerm IS NULL OR :searchTerm = '' OR type LIKE concat('%', :searchTerm, '%'))")
     ...
     * 统计今日新增
     */
    @Query(value = "SELECT count(*) FROM risk_data WHERE toDate(insert_time) = toDate(now())", nativeQuery = true)
    long countTodayIncrease();

    /**
     * 按天统计一周内新增
     */
    @Query(nativeQuery = true, value = "SELECT toStartOfDay(insert_time) AS group_key, COUNT(*) AS count FROM risk_data WHERE insert_time >= today() - INTERVAL 1 WEEK GROUP BY toStartOfDay(insert_time) ORDER BY group_key")
    List<Map<String, Object>> countByDateOfWeek();

    /**
     * 按风险等级统计
     */
    @Query(nativeQuery = true, value = "select risk_level as group_key, count(*) as count from risk_data GROUP BY risk_level")
