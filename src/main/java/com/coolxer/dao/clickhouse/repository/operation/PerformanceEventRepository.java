 ...
 * 提供性能事件的增删改查等基础操作
 */
public interface PerformanceEventRepository extends JpaRepository<PerformanceEvent, String> {

    @Query(nativeQuery = true,
            value = "SELECT * FROM operation_performance_event ORDER BY update_time ASC",
            countQuery = "SELECT count(*) FROM operation_performance_event")
    Page<PerformanceEvent> findByPage(Pageable pageable);

    @Query(nativeQuery = true,
            value = "SELECT * FROM operation_performance_event WHERE " +
            ...
            countQuery = "SELECT count(*) FROM operation_performance_event WHERE " +
                    "(:id IS NULL OR :id = '' OR id = :id) AND " +
                    "(:userId IS NULL OR :userId = '' OR user_id = :userId) AND " +
                    "(:startId IS NULL OR :startId = '' OR start_id = :startId) AND " +
                    "(:netType IS NULL OR :netType = '' OR net_type = :netType) AND " +
                    ...
                    "(:updateTimeStart IS NULL OR :updateTimeEnd IS NULL OR (update_time >= :updateTimeStart AND update_time <= :updateTimeEnd)) AND " +
                    "(:insertTimeStart IS NULL OR :insertTimeEnd IS NULL OR (insert_time >= :insertTimeStart AND insert_time <= :insertTimeEnd))")
    Page<PerformanceEvent> findByPageAsc(Pageable pageable,
                                         @Param("id") String id,
    ...
    @Query(nativeQuery = true,
            ...
            countQuery = "SELECT count(*) FROM operation_performance_event WHERE " +
                    ...
                    "(:updateTimeStart IS NULL OR :updateTimeEnd IS NULL OR (update_time >= :updateTimeStart AND update_time <= :updateTimeEnd)) AND " +
                    "(:insertTimeStart IS NULL OR :insertTimeEnd IS NULL OR (insert_time >= :insertTimeStart AND insert_time <= :insertTimeEnd))")
    Page<PerformanceEvent> findByPageDesc(Pageable pageable,
                                          @Param("id") String id,
                                          @Param("userId") String userId,
                                          @Param("startId") String startId,
                                          @Param("netType") String netType,
    ...
    @Query(nativeQuery = true,
            value = "SELECT DISTINCT id FROM operation_performance_event WHERE id IS NOT NULL AND (:searchTerm IS NULL OR :searchTerm = '' OR id LIKE concat('%', :searchTerm, '%'))")
    ...
    @Query(value = "SELECT count(*) FROM operation_performance_event WHERE toDate(insert_time) = toDate(now())", nativeQuery = true)
    long countTodayIncrease();

    @Query(nativeQuery = true, value = "SELECT toStartOfDay(insert_time) AS group_key, COUNT(*) AS count FROM operation_performance_event WHERE insert_time >= today() - INTERVAL 1 WEEK GROUP BY toStartOfDay(insert_time) ORDER BY group_key")
